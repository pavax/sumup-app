...
export class TransactionDetailsDialogComponent {
  ...
  constructor(
    ...
    private store: Store,
    @Inject(MAT_DIALOG_DATA) private data: TransactionViewModel
  ) {
  ...
  private createFirstEntry(transactionViewModel: TransactionViewModel) {
    if (!transactionViewModel.failed) {
      return {
        ...
        timestamp: new Date(
          transactionViewModel.origTransactionTimestamp
        ).getTime(),
        status: 'success',
      };
    } else {
  ...
  private convertEvents(events: Event[]) {
    return events.map(event => {
      ...
      if (event.type === EventType.REFUND) {
        ...
        event.type === EventType.PAYOUT &&
        event.status === EventStatus.PAID_OUT
      ) {
