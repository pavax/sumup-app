...
export class TransactionsEffects {
    ...
    return this.actions$.pipe(
      ...
      switchMap(action =>
        this.transactionsApiService
          ...
          .pipe(
            map(response =>
              TransactionsActions.fetchTransactionsSuccess({ data: response })
            ),
            catchError(error =>
              of(TransactionsActions.fetchTransactionsFailure({ error }))
            )
          )
      )
    );
  ...
  loadMoreTransactions$ = createEffect(() => {
    return this.actions$.pipe(
      ...
      withLatestFrom(
        this.store.select(TransactionSelector.selectCurrentFilter),
        this.store.select(TransactionSelector.selectOldestTx)
      ),
      switchMap(([, filter, oldestTx]) => {
        return this.transactionsApiService
          .listTransactions(
            filter?.dateFrom,
            filter?.dateTo,
            filter?.statuses,
            oldestTx.transaction_id
          )
          .pipe(
            map(response =>
            ...
            catchError(error =>
              of(TransactionsActions.loadMoreTransactionsFailure({ error }))
            )
          );
      })
    );
  ...
  dispatchFetchTransactionDetails$ = createEffect(() => {
    return this.actions$.pipe(
      ...
      map(action => {
        const ids = action.data.map(r => r.transaction_id);
        if (action.type === TransactionsActions.fetchTransactionsSuccess.type) {
          return TransactionsActions.fetchTransactionDetails({
            transactionIds: ids,
            trigger: 'FILTER',
          });
        }
    ...
    return this.actions$.pipe(
      ...
      switchMap(action => {
        ...
        if (transactionIds.length === 0) {
          return of(
            TransactionsActions.fetchTransactionDetailsSuccess({ data: [] })
          );
        }
        return forkJoin(
          transactionIds.map(id =>
            this.transactionsApiService.loadTransactionDetail(id)
          )
        ).pipe(
          map(response =>
            TransactionsActions.fetchTransactionDetailsSuccess({
              data: response,
            })
          ),
          catchError(error =>
            of(TransactionsActions.fetchTransactionDetailsFailure({ error }))
          )
        );
      })
    );
